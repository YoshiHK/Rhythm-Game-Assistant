...
def update_runtime_dbs(
    *,
    ...
    # --------------------------------------------------
    # Debug print limits (print only; no effect on counters)
    # --------------------------------------------------
    ...
    for i, path in enumerate(files):
        ...
        if normalize_folder_identity is None:
            ...
            if debug_route_miss_count < DEBUG_LIMIT_ROUTE_MISS:
                print(
                    f"[DEBUG ROUTE_MISS:NORMALIZER_UNAVAILABLE] "
                    f"path={_safe_log_text(path)} "
                    f"hier={_safe_log_text(hier)}"
                )
        ...
        if not game_id:
            ...
            if debug_route_miss_count < DEBUG_LIMIT_ROUTE_MISS:
                print(
                    f"[DEBUG ROUTE_MISS:GAME_ID_MISSING] "
                    f"path={_safe_log_text(path)} "
                    f"hier={_safe_log_text(hier)} "
                    f"norm={_safe_log_text(norm)}"
                )
                debug_route_miss_count += 1
        ...
        try:
            ...
            if debug_route_miss_count < DEBUG_LIMIT_ROUTE_MISS:
                print(
                    f"[DEBUG ROUTE_MISS:GET_ADAPTER_FAILED] "
                    f"game_id={_safe_log_text(game_id)} "
                    f"path={_safe_log_text(path)} "
                    f"error_type={type(e).__name__} "
                    f"error_msg={_safe_log_text(e)}"
                )
                debug_route_miss_count += 1
        ...
        try:
            ...
            if debug_route_miss_count < DEBUG_LIMIT_ROUTE_MISS:
                print(
                    f"[DEBUG ROUTE_MISS:GET_VALIDATOR_FAILED] "
                    f"game_id={_safe_log_text(game_id)} "
                    f"path={_safe_log_text(path)} "
                    f"error_type={type(e).__name__} "
                    f"error_msg={_safe_log_text(e)}"
        ...
        # STEP 4: canonical payload build
        try:
            ...
            if debug_canonical_error_count < DEBUG_LIMIT_CANONICAL_ERROR:
                print(
                    f"[DEBUG CANONICAL_ERROR:TRY_BUILD_PAYLOAD_FAILED] "
                    f"game_id={_safe_log_text(game_id)} "
                    f"path={_safe_log_text(path)} "
                    f"adapter_id={_safe_log_text(getattr(adapter, 'adapter_id', None))} "
                    f"error_type={type(e).__name__} "
                    f"error_msg={_safe_log_text(e)}"
                )
            ...
            _append_canonical_error(
                ...
                reason_message="_try_build_payload(adapter, path) raised",
                exception=e,
                validation_ok=None,
                canonical_payload_ok=False,
                canonical_row_ok=None,
            )
            continue
        ...
        try:
            ...
            if debug_canonical_error_count < DEBUG_LIMIT_CANONICAL_ERROR:
                print(
                    f"[DEBUG CANONICAL_ERROR:VALIDATOR_RAISED] "
                    f"game_id={_safe_log_text(game_id)} "
                    f"path={_safe_log_text(path)} "
                    f"validator_id={_safe_log_text(getattr(validator, 'validator_id', None))} "
                    f"error_type={type(e).__name__} "
                    f"error_msg={_safe_log_text(e)}"
                )
                debug_canonical_error_count += 1
        ...
        try:
            ...
            for ev in note_events:
                if isinstance(ev, dict):
                    k = ev.get("kind")
                    if isinstance(k, str):
                        observed_kinds.add(k)
        except Exception:
            pass

        semantic_missing = [
            k for k in observed_kinds
        ...
        if validation_ok is False:
            ...
            final_bucket = _BUCKET_VALIDATION_FAIL

            if semantic_missing:
                final_bucket = _BUCKET_SEMANTIC_GAP

            elif coverage and semantic_level != "high":
                final_bucket = _BUCKET_COVERAGE_GAP

            else:
                final_bucket = _BUCKET_VALIDATOR_FAIL

            if debug_canonical_error_count < DEBUG_LIMIT_CANONICAL_ERROR:
                print(
                    f"[DEBUG CANONICAL_ERROR:VALIDATION_NOT_OK] "
                    f"game_id={_safe_log_text(game_id)} "
                    f"path={_safe_log_text(path)} "
                    f"classified_bucket={final_bucket} "
                    f"semantic_missing={_safe_log_text(semantic_missing)} "
                    f"semantic_level={semantic_level}"
                )
                debug_canonical_error_count += 1
        ...
        try:
            ...
            if debug_canonical_error_count < DEBUG_LIMIT_CANONICAL_ERROR:
                print(
                    f"[DEBUG CANONICAL_ERROR:TO_CANONICAL_ROW_FAILED] "
                    f"game_id={_safe_log_text(game_id)} "
                    f"path={_safe_log_text(path)} "
                    f"adapter_id={_safe_log_text(getattr(adapter, 'adapter_id', None))} "
                    f"error_type={type(e).__name__} "
...
def cli_main(argv: Optional[List[str]] = None) -> int:
    ...
    canonical_error_out: Optional[Path] = (
        Path(args.canonical_error_out) if args.canonical_error_out else None
    )
    
    runtime_baseline_out: Optional[Path] = (
        Path(args.runtime_baseline_out)
        if args.runtime_baseline_out
        else None
    )    
    ...
    if json_out is not None:
        try:
            ...
            print(
                f"[DB UPDATE ERROR] Failed to write json_out "
                f"path={json_out} error={type(e).__name__}: {e}"
            )
            raise
            
    # --------------------------------------------------
    # Runtime Baseline Contract (optional)
    # --------------------------------------------------
        ...
        baseline = {
            ...
            "databases": {
                "file_scan_inventory.db":
                    _db_snapshot(inventory_db),

                "chart_assets.db":
                    _db_snapshot(asset_db),

                "chart_patterns.db":
                    _db_snapshot(pattern_db),
            },
