...
def normalize_game(value: Optional[str]) -> Optional[str]:
    ...
    canonical = _canonicalize_game_token(token)

    if canonical in GAME_DIFFICULTY_CAPABILITIES:
        return canonical

    # keep docstring-consistent behavior
...
def normalize_difficulty(value: Optional[str]) -> Optional[str]:
    ...
    # Small canonical cleanup before alias lookup
    token = token.replace("re master", "remaster")
...
def normalize_folder_identity(
    *,
    ...
    This function:
    - Normalizes game / difficulty / level independently
    - Supports combined difficulty+level folder patterns
    - Validates compatibility (non-destructive)
    - Surfaces issues instead of mutating or guessing
    """

    issues: List[str] = []

    # ---------------------------------------------------------
    # Step 1: Normalize basic components
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # Step 2: Handle combined difficulty+level patterns
    # ---------------------------------------------------------

    combined = _extract_combined_difficulty_and_level(
        difficulty_folder=difficulty_folder,
        level_folder=level_folder,
    )

    combined_diff = combined.get("difficulty")
    combined_level = combined.get("level")

    # Prefer combined extraction if available
    if combined_diff is not None:
        difficulty = combined_diff
    ...
    # ---------------------------------------------------------
    # Step 3: Validation (non-destructive)
    # ---------------------------------------------------------

    validation_issue = validate_game_difficulty(
        game=game,
    ...
    # ---------------------------------------------------------
    # Step 4: Missing signal checks (soft issues)
    # ---------------------------------------------------------

    if game_folder and game is None:
        issues.append("unrecognized_game_folder")

    if difficulty_folder and difficulty is None:
        issues.append("unrecognized_difficulty_folder")

    if level_folder and level is None:
        issues.append("unrecognized_level_folder")

    # ---------------------------------------------------------
    # Step 5: Return structured identity
    # ---------------------------------------------------------
    ...
    return {
        ...
        # raw inputs (critical for debugging + verification layer)
        "raw": {
