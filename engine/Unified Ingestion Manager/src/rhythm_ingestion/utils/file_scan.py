...
def cli_main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.cmd == "commit":
        ...
        # ------------------------------------------
        # 3) asset ingestion (clean version + hierarchy repair ✅)
        # ------------------------------------------
        try:
        ...
        def _clean_folder_value(v: Any) -> Optional[str]:
            if v is None:
                return None
            s = str(v).strip()
            return s or None
        ...
        def _repair_hierarchy(
            hier: Dict[str, Any],
            inferred_game_id: Optional[str],
        ) -> Dict[str, Optional[str]]:
            """
            Lightweight wiring-level repair only.
            Do NOT invent semantics; only fix obvious positional pollution.
            """
            game_folder = _clean_folder_value(hier.get("game_folder"))
            difficulty_folder = _clean_folder_value(hier.get("difficulty_folder"))
            level_folder = _clean_folder_value(hier.get("level_folder"))
            ...
            inferred_game_folder = (
                inferred_game_id.replace("_", " ") if inferred_game_id else None
            )

            # Case A:
            #   Chart File / bandori / Special
            # -> bandori / Special / None
            if game_folder and _cf(game_folder) in root_noise:
                if inferred_game_folder and difficulty_folder:
                    if _cf(difficulty_folder) == _cf(inferred_game_folder):
                        game_folder = difficulty_folder
                        difficulty_folder = level_folder
                        level_folder = None
                    else:
                        game_folder = inferred_game_folder

            # Case B:
            # If extracted game mismatches inferred game but difficulty matches inferred game,
            # treat it as a shifted hierarchy.
            if inferred_game_folder and difficulty_folder:
                if _cf(difficulty_folder) == _cf(inferred_game_folder):
                    if not game_folder or _cf(game_folder) != _cf(inferred_game_folder):
                        game_folder = difficulty_folder
                        difficulty_folder = level_folder
                        level_folder = None
        ...
        if ingest_chart_assets_from_file_scan_candidates:
            try:
                ...
                for p in candidates:
                    ...
                    repaired_hier = _repair_hierarchy(
                        hier=hier,
                    ...
                    try:
                        norm = normalize_folder_identity(
                            game_folder=repaired_hier.get("game_folder"),
                            difficulty_folder=repaired_hier.get("difficulty_folder"),
                            level_folder=repaired_hier.get("level_folder"),
                        )
                    except Exception:
                        skipped_unroutable += 1
                        continue

                    # Only pass routable candidates into asset ingestion.
                    # This is wiring-level filtering, not phase redesign.
                    if not norm.get("game") or not norm.get("difficulty"):
                        skipped_unroutable += 1
                        continue
                    ...
                    asset_candidates.append(
                        {
                            ...
                            # normalized identity (non-destructive additive fields)
                            "normalized_game": norm.get("game"),
                            "normalized_difficulty": norm.get("difficulty"),
                            "normalized_level": norm.get("level"),
                            "normalization_issues": norm.get("issues"),
                            ...
                            "extra_metadata": {
                                ...
                                "inferred_from": "path",
                                "inference_version": 1,
                                "repaired_hierarchy": repaired_hier,
                                "normalized_game": norm.get("game"),
                                "normalized_difficulty": norm.get("difficulty"),
                                "normalized_level": norm.get("level"),
                                "normalization_issues": norm.get("issues"),
                ...
                print(f"  candidates_in: {len(asset_candidates)}")
                print(f"  skipped_unroutable: {skipped_unroutable}")
                print(f"  skipped_no_normalizer: {skipped_no_normalizer}")
