...
def _looks_like_chunithm_path(path: Path) -> bool:
    ...
    return any(h in s for h in hints) or any(h in name for h in hints)

# ----------------------------
# Adapter implementation
# ----------------------------

class ChunithmAdapter(BaseAdapterV2):
    game_id = "chunithm"
    adapter_id = "adapter_chunithm"
    adapter_version = "2.0.0"

    def to_canonical_payload(self, path: str) -> Dict[str, Any]:
        payload = build_canonical_payload_chunithm(path)

        # ✅ KEY: normalize contract here (Wave 1.2 core)
        payload = self.finalize_payload_v2(
            payload,
            source_path=str(path),
            default_chart_id=payload.get("chart_id") or str(path),
            default_difficulty=payload.get("difficulty"),
        )
