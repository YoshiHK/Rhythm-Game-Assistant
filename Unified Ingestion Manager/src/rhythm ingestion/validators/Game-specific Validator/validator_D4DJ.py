...
D4DJValidator (UMI Phase 3)

Validates:
- canonical_row minimal sanity
- canonical_payload.note_events structural integrity
- D4DJ-specific invariants for the plain-text chart adapter

Scope (Phase 3 only):
- Structural validation only
- No gameplay semantics, no tips, no Phase 4 logic

Designed to validate payloads emitted by adapter_D4DJ.py.
"""
...
class D4DJValidator(BaseValidator):
    game_id = "D4DJ"
    ...
    def validate(
        ...
        *,
        ...
        if canonical_row.get("game") not in (self.game_id, "D4DJ"):
            errors.append(f"canonical_row['game'] must be 'D4DJ', got {canonical_row.get('game')!r}.")
        ...
        # ----------------------------
        if canonical_payload.get("game_id") not in (None, self.game_id, "D4DJ"):
            errors.append(
                f"canonical_payload['game_id'] mismatch: expected 'D4DJ', got {canonical_payload.get('game_id')!r}."
            )

        note_events = canonical_payload.get("note_events")
        if not isinstance(note_events, list) or not note_events:
            errors.append("canonical_payload['note_events'] must be a non-empty list for D4DJ.")
        ...
        # D4DJ adapter uses time_unit=seconds
        tu = chart_meta.get("time_unit")
        if tu is not None and tu != "seconds":
            errors.append(f"chart_meta.time_unit must be 'seconds' when present, got {tu!r}.")
        ...
        for idx, ev in enumerate(note_events):
            ...
            if not isinstance(kind, str):
                ...
                errors.append(f"note_events[{idx}].kind={kind!r} is not an allowed canonical kind for D4DJ.")
            ...
            for req in ("raw_type", "lane_id_raw", "next_id", "direction", "effect_type", "effect_parameter", "time_raw"):
                if req not in extra:
            ...
            if kind == "hold_path":
                ...
                if dur_f <= 0.0:
                    errors.append(
                        f"note_events[{idx}] kind='hold_path' but extra.duration_seconds={dur!r} is not > 0."
                    )
    ...
    def _fail_if(errors: List[str], *, stage: str, song_id: Any) -> None:
        if errors:
            raise ValueError(
                f"D4DJValidator ({stage}) failed for song_id={song_id!r}: " + "; ".join(errors)
            )
