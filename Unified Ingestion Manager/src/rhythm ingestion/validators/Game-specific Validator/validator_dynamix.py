#!/usr/bin/env python3
"""validator_dynamix.py

DynamixValidator (UMI Phase 3)

Validates:
- canonical_row minimal sanity
- canonical_payload.note_events structural integrity
- Dynamix XML-specific invariants (3 sides, continuous positions, hold pairing)

Scope (Phase 3 only):
- Structural validation only
- No gameplay semantics, no tips, no Phase 4 logic

Designed to validate payloads emitted by adapter_dynamix.py.
"""
...
class DynamixValidator(BaseValidator):
    game_id = "dynamix"
    ...
    def validate(
        ...
        *,
        ...
        if canonical_row.get("game") not in (self.game_id, "dynamix"):
            errors.append(f"canonical_row['game'] must be 'dynamix', got {canonical_row.get('game')!r}.")
        ...
        # ----------------------------
        if canonical_payload.get("game_id") not in (None, self.game_id, "dynamix"):
            errors.append(
                f"canonical_payload['game_id'] mismatch: expected 'dynamix', got {canonical_payload.get('game_id')!r}."
            )

        note_events = canonical_payload.get("note_events")
        if not isinstance(note_events, list) or not note_events:
            errors.append("canonical_payload['note_events'] must be a non-empty list for dynamix.")
        ...
        # time unit is bars
        tu = chart_meta.get("time_unit")
        if tu is not None and tu != "bars":
            errors.append(f"chart_meta.time_unit must be 'bars' when present, got {tu!r}.")

        bpm = chart_meta.get("bpm")
        if not isinstance(bpm, (int, float)) or float(bpm) <= 0:
            errors.append(f"chart_meta.bpm must be a positive number (bar_per_min proxy), got {bpm!r}.")
        ...
        for idx, ev in enumerate(note_events):
            ...
            if not isinstance(kind, str):
                ...
                errors.append(f"note_events[{idx}].kind={kind!r} is not an allowed canonical kind for dynamix.")
            ...
            for req in ("raw_type", "id", "side", "position", "width", "sub_id"):
                if req not in extra:
                    errors.append(f"note_events[{idx}].extra missing {req!r}.")

            if kind == "hold_path":
                dur = extra.get("duration_bars")
                try:
                    dur_f = float(dur)
                except Exception:
                    dur_f = -1.0
                if dur_f <= 0:
                    errors.append(
                        f"note_events[{idx}] kind='hold_path' but extra.duration_bars={dur!r} is not > 0."
                    )
    ...
    def _fail_if(errors: List[str], *, stage: str, song_id: Any) -> None:
        if errors:
            raise ValueError(
                f"DynamixValidator ({stage}) failed for song_id={song_id!r}: " + "; ".join(errors)
            )
