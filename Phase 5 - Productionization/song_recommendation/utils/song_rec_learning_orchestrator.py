...
def _ensure_import_paths() -> None:
    """
    Ensure all Phase 5 subfolders are importable in script mode.
    """
    import sys
    from pathlib import Path
    ...
    song_rec_root = utils_dir.parent                      # song_recommendation/
    phase5_root = song_rec_root.parent                    # Phase 5 - Productionization/
    repo_root = phase5_root.parent                        # Github repo root
    ...
    sub_dirs = [
        song_rec_root / "aggregation",
        song_rec_root / "features",
        song_rec_root / "training",        
        song_rec_root / "evaluation",
        song_rec_root / "artifacts",
        song_rec_root / "utils",
    ]

    for p in base_paths + sub_dirs:
        ps = str(p)
...
def _ensure_import_paths() -> None:
    ...
    are importable when this orchestrator is executed directly as a script.

    Canonical policy:
    - Primary import root: song_recommendation.*
    - Package exports are secondary
    - Flat imports are last-resort only
    - Legacy phase5.* is optional final fallback
    """
    this_file = Path(__file__).resolve()

    utils_dir = this_file.parent                         # .../song_recommendation/utils
    song_rec_root = utils_dir.parent                     # .../song_recommendation
    phase5_root = song_rec_root.parent                   # .../Phase 5 - Productionization
    repo_root = phase5_root.parent                       # .../Github Repository

    # Canonical package roots
    for p in (repo_root, phase5_root, song_rec_root):
        ps = str(p)
        if ps not in sys.path:
            sys.path.insert(0, ps)

    # Optional subdir additions for script-mode fallbacks
    for sub in (
        song_rec_root / "aggregation",
        song_rec_root / "features",
        song_rec_root / "training",
...
def _imports():
    ...
    Canonical policy:
    1) direct module imports from song_recommendation.<layer>.<module_file>
    2) package-export fallback from song_recommendation.<layer>
    3) flat/script fallback
    4) legacy phase5.* fallback
    ...
    # --------------------------------------------------
    # 1) Preferred: direct module-file imports
    # --------------------------------------------------
    try:
        from song_recommendation.aggregation.aggregate_song_feedback import (
            aggregate_song_feedback_events,
        )
        from song_recommendation.features.selection_features import (
            build_selection_feature_rows,
        )
        from song_recommendation.training.train_selector_params import (
            train_song_selector_params,
        )
        from song_recommendation.evaluation.evaluate_selection_quality import (
            evaluate_selection_quality,
        )

        try:
            from song_recommendation.artifacts.artifact_exporter import (
                ArtifactPaths,
                write_song_selector_params,
                write_training_report,
                write_evaluation_report,
            ...
            from song_recommendation.artifacts import (
                ArtifactPaths,
                write_song_selector_params,
                write_training_report,
                write_evaluation_report,
        ...
        errors.append(f"[layout-1 direct song_recommendation.<layer>.<file>] {type(e).__name__}: {e}")

    # --------------------------------------------------
    # 2) Package-export fallback
    #    Only works if each package __init__.py exports the right symbols
    # --------------------------------------------------
    try:
        from song_recommendation.aggregation import (
            aggregate_song_feedback_events,
        )
        from song_recommendation.features import (
            build_selection_feature_rows,
        )


        from song_recommendation.training import (
            train_song_selector_params,
        )
        ...
        errors.append(f"[layout-2 package exports] {type(e).__name__}: {e}")
    ...
    try:
        ...
        errors.append(f"[layout-3 flat module fallback] {type(e).__name__}: {e}")

    # --------------------------------------------------
    # 4) Legacy phase5.* fallback
    # --------------------------------------------------
    try:
        from phase5.song_recommendation.aggregation.aggregate_song_feedback import (
            aggregate_song_feedback_events,
        )
        from phase5.song_recommendation.features.selection_features import (
            build_selection_feature_rows,
        )
        from phase5.song_recommendation.training.train_selector_params import (
            train_song_selector_params,
        )
        from phase5.song_recommendation.evaluation.evaluate_selection_quality import (
            evaluate_selection_quality,
        )

        try:
            from phase5.song_recommendation.artifacts.artifact_exporter import (
                ArtifactPaths,
                write_song_selector_params,
                write_training_report,
                write_evaluation_report,
        ...
        errors.append(f"[layout-4 legacy phase5.*] {type(e).__name__}: {e}")

    # --------------------------------------------------
    # Final hard failure with full diagnostics
    # --------------------------------------------------
    raise ImportError(
        "Unable to import Phase 5 pipeline dependencies from any layout.\n"
        + "\n".join(errors)
    )
